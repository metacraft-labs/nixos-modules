...
SummaryTableEntry[] convertNixEvalToTableSummary(Package[] packages)
{
    return packages
        ...
        .map!(group => createSummaryTableEntry(group.array))
        .array
        .sort!((a, b) => a.name < b.name)
        .release;
}

SummaryTableEntry createSummaryTableEntry(Package[] group)
{
    JSONValue pkg;
    ...
    SummaryTableEntry entry = {
        name, {getStatus(pkg, "x86_64_linux"), getStatus(pkg, "x86_64_darwin")}, {
