...
* Introduction
  ...
  approve. Humans review the plan comment and merge into the target
  environment branch. This separation-of-duties principle underlies all CI
  safety gates below.
  ...
  - Branch protection on deployment branches (required reviews + required status checks);
    sensitive paths (bootstrap/, backends/, workflow files, .terraform.lock.hcl,
    CODEOWNERS) require 2 human approvals
  - GitHub Environments configured as apply credential boundaries. Required
    Environment reviewers are optional project-specific exceptions; the default
    approval path is merging into the target deployment branch.
...
:END:
  ...
  - blocksense/infra M0 requires: fork-PR guard, denylist, plan safety gate,
    ...
    Environment credential boundary, post-apply smoke tests (as hook/step),
  ...
  job's credentials are only available in the GitHub Environment associated
  with the target deployment branch.
...
*** Deliverables
    ...
    - [X] Apply-on-merge job: runs tofu apply with a GitHub Environment
          credential boundary; uses dflook/tofu-apply which re-generates the plan and compares
    ...
    - [X] Read-write credentials for apply job (only in the target GitHub Environment)
    - [X] Apply GitHub Environment name (defaults to production; consuming repos
          can map branches such as testing/staging/main to matching environments)
...
*** Verification
    ...
    - test_name: verify_apply_environment_boundary
      description: Apply job runs inside the configured GitHub Environment for the target branch
      status: pending
    ...
    - test_name: verify_end_to_end_consuming_repo
      description: A consuming repo can call the reusable workflow and complete the full pipeline — offline checks run, credentialed plan posts a comment, fork-PR guard skips credentials for fork PRs, merge to the target branch applies through that branch's GitHub Environment
      status: pending
...
*** Outstanding Tasks
    ...
    - [X] Implement plan-as-PR-comment using dflook/tofu-plan (not dflook/terraform-plan);
          ensure plan-as-PR-comment output serves as the pre-merge preview for the
