...
Cachix features (the ~mcl-devops deploy-spec~ command and ~mcl.utils.cachix~, the
...
* DONE  Cachix Deploy usage removed
  The production activation path is Attic-only (~mcl-devops deploy-ssh~ /
  ~mcl-devops deploy-reconcile~ / the pull-agent + forced-command ssh-apply). The
