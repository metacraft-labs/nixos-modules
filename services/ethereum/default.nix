...
in {
  imports = [
    (import "${dirs.lib}/import-agenix.nix" "ethereum")
  ];
  services.ethereum.erigon.${network} = {
    ...
    args = {
      chain = "${network}";
      authrpc.jwtsecret = config.age.secrets."ethereum/erigon-goerli/jwtSecret".path;
      ${rpcProtocol} = {
        enable = true;
        addr = "0.0.0.0";
        port = rpcPort;
        api = ["eth" "erigon" "engine" "sealer" "net" "web3"];
  ...
  services.ethereum.nimbus-eth2."${network}" = {
    enable = true;
    openFirewall = true;
    args = {
      inherit network;
      ...
      web3-urls = ["${rpcProtocol}://localhost:${toString rpcPort}/"];
      rest.enable = true;
      jwt-secret = config.age.secrets."ethereum/nimbus-eth2/jwtSecret".path;
      trusted-node-url = "http://unstable.prater.beacon-api.nimbus.team/";
      backfill = false;
    };
  };
  ...
  services.prometheus.scrapeConfigs = [
    {
      job_name = "nimbus-eth2";
      static_configs = [
        {
          targets = ["127.0.0.1:${toString config.services.ethereum.nimbus-eth2.goerli.args.metrics.port}"];
        }
      ];
    }
  ];
