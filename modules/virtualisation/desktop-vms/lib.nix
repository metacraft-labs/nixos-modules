...
let
  ...
  attrsToXmlAttrs = attrs:
    concatStringsSep " " (mapAttrsToList (k: v:
      if v == null then ""
      else if isBool v then (if v then "${k}=\"yes\"" else "${k}=\"no\"")
      else "${k}=\"${toString v}\""
    ) attrs);
  ...
  xmlElement = name: attrs: content:
    ...
    then "<${name}${attrPart}/>"
    else "<${name}${attrPart}>${content}</${name}>";

  # Render XML element with nested children
  xmlElementNested = name: attrs: children:
    let
      attrStr = attrsToXmlAttrs attrs;
      attrPart = optionalString (attrStr != "") " ${attrStr}";
      childrenStr = if isList children
...
rec {
  ...
  parseMemory = memStr:
    ...
    in
    if matches == null
    then throw "Invalid memory format: ${memStr}. Expected format: 8G, 16GiB, 8192M, etc."
    else {
      ...
      in
        if lib.hasPrefix "g" normalized then "GiB"
        else if lib.hasPrefix "m" normalized then "MiB"
        else if lib.hasPrefix "k" normalized then "KiB"
        else "GiB"; # Default to GiB
    };
  ...
  parseDiskSize = sizeStr:
    ...
    in
    if matches == null
    then throw "Invalid disk size format: ${sizeStr}. Expected format: 100G, 500GiB, 1T, etc."
    else {
      count = lib.strings.toInt (builtins.elemAt matches 0);
      unit = let
        unitStr = builtins.elemAt matches 1;
        normalized = lib.strings.toLower unitStr;
      in
        if lib.hasPrefix "t" normalized then "T"
        else if lib.hasPrefix "g" normalized then "G"
        else if lib.hasPrefix "m" normalized then "M"
        else "G";
    };
  ...
  generateCpuPinningXml = cpuPinning:
    if cpuPinning == null then ""
    else
      let
        pinnings = lib.imap0 (i: cpuset: xmlElement "vcpupin" { vcpu = i; inherit cpuset; } null) cpuPinning;
      in
  ...
  generateMemoryBackingXml = { hugepages ? false, shared ? false }:
    let
      children = []
    ...
    in
    if children == [] then ""
    else xmlElementNested "memoryBacking" {} children;
  ...
  generateVirtioFsXml = sharedFolders:
  ...
  generateSpiceGraphicsXml = { port ? 5900, listen ? "127.0.0.1", autoport ? true }:
    ''
      <graphics type="spice" port="${toString port}" autoport="${if autoport then "yes" else "no"}" listen="${listen}">
  ...
  generateVncGraphicsXml = { port ? 5900, listen ? "127.0.0.1", autoport ? true }:
    ''
      <graphics type="vnc" port="${toString port}" autoport="${if autoport then "yes" else "no"}" listen="${listen}">
        <listen type="address" address="${listen}"/>
      </graphics>'';

  # Generate Looking Glass shared memory configuration
  # Reference: https://looking-glass.io/docs/stable/install_host/libvirt/
  generateLookingGlassXml = { memoryMB ? 64 }:
    ''
  ...
  generateSecureBootXml = { enable ? true, ovmfCodePath, ovmfVarsPath, nvramPath }:
    if enable then
      ''
        <os>
  ...
  generateNetworkXml = { bridge ? "virbr0", mac ? null }:
    let
  ...
  generateDiskXml = { pool ? "default", volume, bus ? "virtio", cache ? "writeback" }:
    ''
  ...
  generateDomainXml = {
    ...
    let
      ...
      featuresXml = if osType == "windows" then
        ''
          <features>
            <acpi/>
            <apic/>
      ...
      # Clock configuration
      clockXml = if osType == "windows" then
        ''
      ...
      graphicsXml =
        if display == "spice" then generateSpiceGraphicsXml {}
        else if display == "vnc" then generateVncGraphicsXml {}
        ...
        else throw "Unknown display type: ${display}";
      ...
      # Disk
      diskXml = generateDiskXml { pool = diskPool; volume = diskVolume; };
