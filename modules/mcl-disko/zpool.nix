...
let
  translateDataSetToDiskoConfig =
    dataset@{ lib, snapshot, ... }:
    lib.recursiveUpdate dataset {
      type = "zfs_fs";
      options = {
        "com.sun:auto-snapshot" = if snapshot then "on" else "off";
        canmount = "on";
      };
    };

  restructuredDatasets = builtins.mapAttrs translateDataSetToDiskoConfig poolExtraDatasets;
in
{
  ${poolName} = {
    type = "zpool";
